...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \EndFor
    \State \textbf{return} $fsi > 0$\Comment{No need to divide, since this $fsi \in \{-|G|,0,|G|\}$.}
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
  ...
  \label{alg:comp_q}
  \begin{algorithmic}
    \State $n \gets \text{Exponent}(G)$\Comment{Either compute fresh or let it be be given.}
    \If{$4|n$}
      \State $c \gets 4$
    \Else
      \State $primes \gets \text{PrimeDivisors}(n)$\Comment{GAP function; gives the primes in ascending order.}
      ...
      \EndIf
    \EndIf
    \State $Q \gets \text{NullMat}(d, d)$\Comment{A $d\times d$ zero matrix, so we enter the loop.}
    \State $a \gets 1$
    \While{$\text{det}(Q) = 0$}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \For{$power \in [0, 1, \ldots, k]$}
      \State $\text{Add}(conversion\_matrix, \text{CoeffsCyc}(symmetric^{power}, d))$\Comment{Row of coefficients.}
    \EndFor
    \State $\mathbf{b} \gets \text{SolutionMat}(conversion\_matrix^T, \text{CoeffsCyc}(\mu, d))$\Comment{Vector of $b_i$'s.}
    ...
    \For{$power \in [1, 2, \ldots, k]$}
      ...
      \ElsIf{$c<0$}
        \State $\text{Append}(s, c^*z \string^ power)$ \Comment{We omit the negative sign, as $\text{String}(c)$ has it.}
      \EndIf
    \EndFor
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \For{$i \in [1, \ldots, d]$}\Comment{GAP is $1$-indexed. Also, we use $\text{det}(\mathbf{M}) \neq 0$.}
      ...
      \State $maybe\_solution \gets \text{SolutionMat}(remaining, rhs\_vector)$\Comment{Solve the rest.}
      \If{$maybe\_solution \neq \text{fail}$} \Comment{GAP keyword `fail' signalizes there was no solution.}
        \State $\mathbf{v} \gets maybe\_solution[\{1, \ldots, i-1\}]$
        \State $\text{Add}(\mathbf{v}, 1)$ \Comment{Insert the $1$ back into the appropriate spot.}
...
\begin{algorithm}
  ...
  % \label{alg:ARR}
  \begin{algorithmic}
    \State $indicators \gets \text{Indicator}(\text{CharacterTable}(G), 2)$ \Comment{All the FSI's.}
    \State $characters \gets \text{Irr}(G)$ \Comment{All the irreducible characters.}
    \State $real\_reps \gets [~]$ \Comment{List for storing the real representations.}
    \For{$i \in [1, \ldots, \text{Size}(indicators)]$}\Comment{Equivalently, $i \in [1, \ldots, \text{Size}(characters)]$.}
      \If{$indicators[i] = 1$} \Comment{We \textit{can} realise over reals.}
        \State $gens\_G \gets \text{GeneratorsOfGroup}(G)$
        \State $char \gets characters[i]$
        \State $d \gets char[1]$
        \If{$d = 1$} \Comment{The character itself \textit{is} a real representation.}
          \State $\text{Add}(real\_reps, \rho: g \mapsto char(g))$ \Comment{Defining $\rho$ for $g \in gens\_G$ suffices.}
          \State \color{blue} \Comment{Since we are working with matrices, it should actually be $\rho(g) = [[char(g)]]$.} \color{black}
          \State \textbf{continue}
        \EndIf
        \State $arep \gets \text{IrreducibleAffordingRepresentation}(char)$
        \State $matrix\_gens \gets \text{GeneratorsOfGroup}(\text{Image}(arep))$
        \State $P \gets \text{KroneckerTrick}(d, matrix\_gens)$ \Comment{Using \cref{alg:nonzero} on all generators.}
        ...
        \Else
          ...
          \State $norm\_root \gets \text{PariNorms}(c, \mu)$
          \If{$norm\_root = \text{fail}$} \Comment{In this case, there may not be a norm root.}
            \State $norm\_root \gets \text{PariNorms}(n, \mu)$ \Comment{With $G$ exponent it is guaranteed success.}
          \EndIf
          \State $P \gets P / norm\_root$
        \EndIf
        \State $Q \gets \text{compute\_Q}(P, n, d)$ \Comment{Using \cref{alg:comp_q} for this.}
        \State $\text{Add}(real\_reps, \rho: g \mapsto Q^{-1} \cdot arep(g) \cdot Q)$
      \EndIf
    \EndFor
    \State \textbf{return} $real\_reps$
  \end{algorithmic}
\end{algorithm}
\footnotetext{More efficiently, we can compute $\mu = \sum_{1}^{d}p_{1,i} \cdot \overline{p_{i,1}}$}}
