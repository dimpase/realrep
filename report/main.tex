...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \For{$power \in [1, 2, \ldots, k]$}
      ...
      \ElsIf{$c<0$}
        \State $\text{Append}(s, \text{``c*z^power''})$ \Comment{We omit the negative sign, as $\text{String}(c)$ has it.}
      \EndIf
    \EndFor
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \For{$i \in [1, \ldots, d]$}\Comment{GAP is $1$-indexed. Also, we use $\text{det}(\mathbf{M}) \neq 0$.}
      ...
      \State \Comment{GAP function that solves the remaining system of linear equations.}
      \break
      \If{$maybe\_solution \neq \text{fail}$} \Comment{GAP keyword `fail' signalizes there was no solution.}
        ...
        \State $\text{Append}(\mathbf{v},maybe\_solution[\{i, \ldots, d^2-1\}])$ \Comment{Put in the rest.}
        \State \textbf{break}
      \EndIf
    \EndFor
